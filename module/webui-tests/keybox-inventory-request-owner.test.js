...
(async () => {
  ...
  assert.deepEqual(JSON.parse(JSON.stringify(context.__testInventory())), [{ id: 'newest', filename: 'newest.xml', scope: 'managed', certificate_serial: '', not_after: '', security_level: 'Unknown', is_rkp: false, has_rsa: false, has_ec: false, algorithms: [] }], 'the newest inventory response must win');

  releaseFirst();
  await first;
  assert.deepEqual(JSON.parse(JSON.stringify(context.__testInventory())), [{ id: 'newest', filename: 'newest.xml', scope: 'managed', certificate_serial: '', not_after: '', security_level: 'Unknown', is_rkp: false, has_rsa: false, has_ec: false, algorithms: [] }], 'a delayed stale inventory response must not overwrite current data');
