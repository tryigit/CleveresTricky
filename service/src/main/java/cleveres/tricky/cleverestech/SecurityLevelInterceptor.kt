...
class SecurityLevelInterceptor : BinderInterceptor() {
    ...
    override fun onPostTransact(
        ...
        return try {
            ...
            // Adaptive StrongBox Routing:
            // If the key was generated by StrongBox but no StrongBox keybox is available,
            // return HARDWARE_TYPE_UNAVAILABLE so clients cleanly fall back to TEE.
            val isStrongBox = metadata.keySecurityLevel == SecurityLevel.STRONGBOX
            if (isStrongBox && !CertHack.hasStrongBoxKeybox(callingUid)) {
                replacement.recycle()
                val errorReply = Parcel.obtain()
                errorReply.writeInt(EX_SERVICE_SPECIFIC)
                errorReply.writeString("StrongBox unavailable")
