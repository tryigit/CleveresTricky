...
<body>
    ...
    <div id="keys" class="content" role="tabpanel" aria-labelledby="tab_keys">
        <div class="panel">
            ...
            <label for="kbContent" style="display:block; font-size:0.8em; margin-bottom:5px; color:#888;">Keybox Content (XML)</label>
    ...
    <script>
        ...
        function validateSpoofInputs() {
             const imei = document.getElementById('inputImei').value.trim();
             const imsi = document.getElementById('inputImsi').value.trim();
             const iccid = document.getElementById('inputIccid').value.trim();
             const wifi = document.getElementById('inputWifiMac').value.trim();
             const bt = document.getElementById('inputBtMac').value.trim();
             const simIso = document.getElementById('inputSimIso').value.trim();

             if (imei && (!/^\d{15}$/.test(imei) || !luhnCheck(imei))) return 'Invalid IMEI (15 digits required)';
             if (imsi && !/^\d{14,15}$/.test(imsi)) return 'Invalid IMSI (14-15 digits required)';
             if (iccid && (!/^\d{19,20}$/.test(iccid) || !luhnCheck(iccid))) return 'Invalid ICCID (19-20 digits required)';

             const macRegex = /^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})$/;
             if (wifi && !macRegex.test(wifi)) return 'Invalid WiFi MAC (XX:XX:XX:XX:XX:XX)';
             if (bt && !macRegex.test(bt)) return 'Invalid BT MAC (XX:XX:XX:XX:XX:XX)';

             if (simIso && !/^[a-z]{2}$/.test(simIso)) return 'Invalid SIM ISO (2 lowercase letters)';

             return null;
        }

        async function applyTemplateToGlobal(btn) {
             const error = validateSpoofInputs();
             if (error) {
                 notify(error, 'error');
                 return;
             }
            ...
            appRules.forEach((rule, idx) => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="text-align:right;">
                        <button class="danger" onclick="removeAppRule(${'$'}{idx})" title="Remove rule" aria-label="Remove rule for ${'$'}{rule.package}">×</button>
                    </td>
                `;
                tbody.appendChild(tr);
            });
