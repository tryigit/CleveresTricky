...
* **Загрузки не перезаписывают файлы:** Перетащенный или вставленный keybox сохраняется как `keybox.xml`; если имя уже занято, автоматически используется следующее свободное имя (`keybox2.xml`, `keybox3.xml`, ...).
