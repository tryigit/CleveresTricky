...
Import يقبل encrypted CTSB فقط ويضع حدودا للرفع وعدد entries/keyboxes والحجم expanded. Traversal وduplicates وdirectories وsymlink destination وsettings/keybox غير الصالحة ترفض قبل الكتابة. Policy v2 تتحقق وتنشر snapshot كاملة. تُضمّن إعدادات الخوادم البعيدة أيضاً بصيغة مشفرة مرتبطة بالجهاز، ولا تُستعاد إلا إذا فُكّ تشفيرها وتحققت على الجهاز نفسه، ومع غياب الإدخال تبقى إعدادات الخوادم الحالية. تطبيق ملف Default يحذف كل الخوادم البعيدة ومحتوى keybox المخزّن مؤقتاً.
