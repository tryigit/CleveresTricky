...
```
This indicates that the device's hardware TEE or vendor KeyMint HAL is failing to communicate (error code -49 / 10). CleveresTricky intentionally trips an internal circuit breaker and halts interception rather than deadlocking the framework. Refer to [Attestation.md](security/Attestation.md#hardware-tee-provisioning--oneplus-recovery-note) for device-side TEE provisioning recovery options (e.g. OnePlus 13/15 unlocked TEE RKP recovery).
