...
sub1(
    policy,
    r'val parsed = parseStateFile\(main, "configured"\)\n\s+publish\(parsed, persistPrevious = false\)',
...
sub1(
    cert,
    r'        final Map<KeyBox, PreparedKeyBox> preparedKeyboxes;\n        final Map<CacheKey, Certificate\[]> certificateCache;',
...
sub1(
    cert,
    r'        State\(Map<String, List<KeyBox>> keyboxes, Map<String, List<KeyBox>> keyboxFiles\) \{.*?\n        \}\n\n        private static Map<String, List<KeyBox>> immutableLists',
...
sub1(
    cert,
    r'    public static boolean canHack\(\) \{\n        return !state\.keyboxes\.isEmpty\(\);\n    \}',
...
sub1(
    cert,
    r'            byte\[] verifiedBootKey = usableBootDigest\(UtilKt\.getBootKey\(\)\);\n            byte\[] verifiedBootHash = usableBootDigest\(UtilKt\.getBootHash\(\)\);',
...
sub1(
    cert,
    r'            ASN1Encodable\[] rootOfTrustEnc = \{.*?            ASN1Sequence hackedRootOfTrust = new DERSequence\(rootOfTrustEnc\);',
    '            ASN1Sequence hackedRootOfTrust = buildLockedRootOfTrust(verifiedBootKey, verifiedBootHash);',
    re.S,
)
sub1(
    cert,
    r'    private static byte\[] usableBootDigest\(byte\[] value\) \{',
...
template = Path("module/template/policy_state_v2.json")
raw = template.read_text()
if '"securityPatch": true' not in raw:
    raise SystemExit("unexpected default policy template")
template.write_text(raw.replace('"securityPatch": true', '"securityPatch": false', 1))

test = "service/src/test/java/cleveres/tricky/cleverestech/PolicyStateHotPathTest.kt"
sub1(
    test,
...
certtest = "service/src/test/java/cleveres/tricky/cleverestech/keystore/CertHackTest.java"
sub1(
    certtest,
...
for path in Path('.github/workflows').glob('attestation-state-hotfix*.yml'):
    path.unlink()
Path('.github/scripts/apply_attestation_state_hotfix.py').unlink()
